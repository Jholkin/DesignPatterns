...
class FlyweightController extends Controller
{
    ...
    public function home(PlayItemFactory $playItemFactory)
    {
        ...
        $playItemFactory->setEnableFlyweight(true);
        echo "Patron Flyweight habilitado? ".$playItemFactory->getEnableFlyweight();
        $this->initArrays();
        $this->createRandomPlayList($playItemFactory);

        echo "<br><br>Total playlist: ",count($this->playLists);
        echo "<br>Memoria usada: ", round(memory_get_usage()/1024, 2), "KB";
    }

    private function createRandomPlayList(PlayItemFactory $playItemFactory)
    {
        $p = 0;
        for ($i=0; $i < count($this->playListNames); $i++) { 
            ...
            for ($j=0; $j < 10; $j++) { 
                $song = rand(100,200);
                $playList->addPlayItem($this->songNames[$song],$playItemFactory);
            }
